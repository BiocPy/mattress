...
lib.py_free_mat.argtypes = [ct.c_void_p, ct.POINTER(ct.c_int), ct.POINTER(ct.c_char_p)]
...
def extract_row(rawmat, r, output):
    errcode___ = ct.c_int(0)
    errmsg___ = ct.c_char_p(0)
    output___ = lib.py_extract_row(
        rawmat, r, output, ct.byref(errcode___), ct.byref(errmsg___)
    )
    if errcode___.value != 0:
        msg = errmsg___.value.decode("ascii")
        lib.free_error_message(errmsg___)
...
def extract_column(rawmat, c, output):
    errcode___ = ct.c_int(0)
    errmsg___ = ct.c_char_p(0)
    output___ = lib.py_extract_column(
        rawmat, c, output, ct.byref(errcode___), ct.byref(errmsg___)
    )
    if errcode___.value != 0:
        msg = errmsg___.value.decode("ascii")
        lib.free_error_message(errmsg___)
...
def initialize_dense_matrix(nr, nc, type, ptr, byrow):
    errcode___ = ct.c_int(0)
    errmsg___ = ct.c_char_p(0)
    output___ = lib.py_initialize_dense_matrix(
        nr, nc, type, ptr, byrow, ct.byref(errcode___), ct.byref(errmsg___)
    )
    if errcode___.value != 0:
        msg = errmsg___.value.decode("ascii")
        lib.free_error_message(errmsg___)
...
def initialize_compressed_sparse_matrix(
    ...
    errcode___ = ct.c_int(0)
    errmsg___ = ct.c_char_p(0)
    output___ = lib.py_initialize_compressed_sparse_matrix(
        nr,
        nc,
        nz,
        dtype,
